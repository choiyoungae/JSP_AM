...
<body>
	...
	<ul>
		<% for(int i=0; i < 3; i++) { %>
		<li><%=(int)articleRows.get(i).get("id") %>번, <%=(LocalDateTime)articleRows.get(i).get("regDate") %>, <%=articleRows.get(i).get("title") %></li>
		<% } %>
	</ul>
	...
	<ul>
		...
		<li><%=(int)articleRow.get("id") %>번, <%=(LocalDateTime)articleRow.get("regDate") %>, <%=articleRow.get("title") %></li>
		<% } %>
	</ul>
</body>
</html>